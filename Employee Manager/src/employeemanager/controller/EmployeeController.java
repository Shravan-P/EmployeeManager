...
public class EmployeeController{
	...
	public ResponseDto<String> insertEmployeeDetailsToDB(Employee employee) {
        logger.info("Inserting employee: {}", employee.getID());
        if (Util.isId(employee.getID()) && Util.isName(employee.getFname(), employee.getLname())
                && Util.isPhn(employee.getphn()) && Util.isEmail(employee.getemail())
                && Util.isJoindate(employee.getJoindate()) && Util.isstatus(employee.getstatus())) {
            try {
                new EmployeeDBService().insertEmployeeDetailsToDBService(employee);
                logger.info("Employee inserted successfully: {}", employee.getID());
                return new ResponseDto<>(StatusConstant.SUCCESS_CODE, null, MessageConstant.CREATE_NEW_EMPLOYEE_SUCCESS);
            } catch (EmployeeServiceException e) {
                logger.error("Service error while inserting employee {}", employee.getID(), e);
                return new ResponseDto<>(StatusConstant.ERROR_CODE, null, e.getMessage());
            } catch (Exception e) {
                logger.error("Unexpected error while inserting employee {}", employee.getID(), e);
                return new ResponseDto<>(StatusConstant.ERROR_CODE, null, e.getMessage());
            }
        } else {
            logger.warn("Invalid employee details for ID: {}", employee.getID());
	...
	public ResponseDto<List<Employee>> getAllEmployeeDetailsFromDB(){
		...
		try {
			...
			logger.error("Employees not found",e);
			return new ResponseDto<>(StatusConstant.ERROR_CODE,null,e.getMessage());
		}
		catch(Exception e) {
	...
	public ResponseDto<Employee> getEmployeeDetailsByIdFromDB(String id){
		...
		try {
			...
			logger.error("Unexpected error fetching employee with ID: {}", id, e);
			return new ResponseDto<>(StatusConstant.ERROR_CODE,null,e.getMessage());
		}
	...
	public ResponseDto<String> deleteEmployeeByIDFromDB(String id) {
        ...
        try {
            ...
            logger.error("Service error deleting employee {}", id, e);
            return new ResponseDto<>(StatusConstant.ERROR_CODE, null, e.getMessage());
        } catch (Exception e) {
            logger.error("Unexpected error deleting employee {}", id, e);
            return new ResponseDto<>(StatusConstant.ERROR_CODE, null, e.getMessage());
        }
    }
	...
	public ResponseDto<String> updateEmployeeByIDInDB(Employee employee) {
        ...
        try {
            ...
            logger.error("Service error updating employee {}", employee.getID(), e);
            return new ResponseDto<>(StatusConstant.ERROR_CODE, null, e.getMessage());
        } catch (Exception e) {
            logger.error("Unexpected error updating employee {}", employee.getID(), e);
            return new ResponseDto<>(StatusConstant.ERROR_CODE, null, e.getMessage());
        }
    }
}
